...
class GaNMNData:
    ...
    def __init__(self, dataset_path: Path) -> None:
        ...
        df[Features.ANSB.value] = df.apply(self._get_artificial_light_pollution_at_row, axis=1)
        print(f"applying cloud cover..")
        df[Features.CLOUD.value] = df.apply(self._get_cloud_cover_at_row, axis=1)
        print(f"applying temperature..")
    ...
    def _sanitize_df(self, gan_frame: pd.DataFrame) -> pd.DataFrame:
        ...
        df: t.Any = gan_frame[gan_frame["nsb"] > 7.]
    ...
    def _encode_dates_in_df(self, gan_mn_frame: pd.DataFrame) -> pd.DataFrame:
        ...
        df[Features.HOUR_SIN.value] = np.sin(2*np.pi*df["received_utc"].dt.hour/HOURS_IN_DAY)
        df[Features.HOUR_COS.value] = np.cos(2*np.pi*df["received_utc"].dt.hour/HOURS_IN_DAY)
        return df.reset_index()
