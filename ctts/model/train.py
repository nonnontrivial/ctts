...
from .build import Features, gan_mn_dir, gan_mn_dataframe_filename
...
model_filename = config.get("file", "model_filename")

path_to_state_dict = gan_mn_dir.parent / model_filename
...
def train_model_at_path(path_to_dataset: Path) -> Dict[str, Any]:
    logging.info(f"loading dataset at {path_to_dataset}")
    ...
    def train():
        ...
        for batch, (X, y) in enumerate(train_dataloader):
            ...
            if batch % 1e4 == 0:
                ...
                logging.info(f"loss: {loss:>7f} [{current:>5d}/{train_dataset_size:>5d}]")
    ...
    def test():
        ...
        with torch.no_grad():
            ...
            logging.info(f"avg test loss: {avg_loss}")

    for t in range(num_epochs):
        logging.info(f"epoch {t+1}/{num_epochs}")
        train()
    test()
